...
async function handlePaidContent(request, env) {
  ...
  // ── VIP bypass ────────────────────────────────────────────────────
  // Set FOLIO_VIP_TOKEN in Cloudflare dashboard -> Workers ->
  // folio-paywall -> Settings -> Variables & Secrets (as a Secret).
  // Value must match the plaintext token in ?unlock= (e.g. SBS-VIP-2026).
  // app.html sends it as X-Folio-VIP header when window.__folioVipBypass is set.
  const vipHeader = (request.headers.get('X-Folio-VIP') || '').trim();
  const vipEnv    = (env.FOLIO_VIP_TOKEN || '').trim();
  const isVip     = !!(vipEnv && vipHeader && vipHeader === vipEnv);
  // ── end VIP bypass ───────────────────────────────────────────────
