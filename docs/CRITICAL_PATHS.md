...
| `/` (welcome page live-stats strip) | anon / any | `collection('folio_projects').where('release.listOnShelf','==',true).where('release.published','==',true).limit(500)` | list branch 2 (`listOnShelf==true`) | Same query shape as /shelf, higher limit for the count. Populates the "N folios · N authors · N reviews" strip beneath the hero. |
| `/` (welcome page reviews count) | anon / any | `collection('reviews').where('approvedForDisplay','==',true).where('allowMarketing','==',true).limit(500)` | reviews `allow read` clause requiring BOTH flags | Both filters mandatory for anon read — the rule engine needs to prove every result satisfies. |
