...
(function (global) {
  ...
  function renderBanner(slot, sel, opts) {
    ...
    // Admin-authored teaserCopy / activeCopy often already starts with
    // the promo label (the default fallback we generate does too). If
    // so, drop the bold label prefix to avoid rendering it twice, e.g.
    // "Read a Book Day — Read a Book Day — 20% off starts in 7 days."
    // Match is case-insensitive and tolerates leading/trailing whitespace.
    const labelText = String(p.label || '').trim();
    const copyText  = String(copy || '').trim();
    const startsWithLabel = labelText.length > 0
      && copyText.toLowerCase().indexOf(labelText.toLowerCase()) === 0;
    const displayLabel = labelText || (teasing ? 'Upcoming discount' : 'Discount live');
    // If copy leads with the label, strip that label (and any following
    // separator like "—", "-", ":", ",") so we can still print a bold
    // prefix without visual duplication.
    let bodyCopy = copyText;
    if (startsWithLabel) {
      bodyCopy = copyText.substring(labelText.length)
        .replace(/^\s*[–—\-:,]\s*/, '')  // drop leading em/en-dash, hyphen, colon, comma
        .trim();
    }
    ...
    slot.innerHTML =
      '<div class="folio-promo-banner" data-state="' + esc(sel.state) + '" data-code="' + esc(p.code) + '"' +
        ...
        '<div style="flex:1;min-width:0">' +
          '<strong style="color:' + accent + '">' + esc(displayLabel) + '</strong>' +
          (bodyCopy ? (' — ' + esc(bodyCopy)) : '') +
          (sel.state === 'active'
